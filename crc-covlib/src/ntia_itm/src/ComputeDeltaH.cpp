// 2023-11-03 Modifications to original NTIA version:
//   - Changed '\' for '/' in following include directive
...
double ComputeDeltaH(double pfl[], double d_start__meter, double d_end__meter)
{
    ...
    // 2023-10-27 Modifications to original NTIA version:
    //   - Added type cast to avoid compiler warning.
    double q10 = diffs[(size_t)(p10 - 1)];

    std::nth_element(diffs.begin(), diffs.begin() + p90, diffs.end(), std::greater<double>());
    // 2023-10-27 Modifications to original NTIA version:
    //   - Added type cast to avoid compiler warning.
    double q90 = diffs[(size_t)p90];
